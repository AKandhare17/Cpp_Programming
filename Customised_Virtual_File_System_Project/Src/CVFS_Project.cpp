...
//defines that file type is Regular file(.txt,.py,.java,.img,.pdf,.docs,.csv  file(mostly regualr use file) etc.)
#define REGULARFILE 1
//defines that file type is specialtype(ex=directory file,.iso,.bat,.sh,.zip,.exe.inc,.dll{/proc,/dev}(system files)  etc.)
...
//if space is not enough while writing
...
void initializeUAREA()
{
    ...
    strcpy(uareaobj.ProcessName,"Myexe");
     ...
     {
        uareaobj.UFDT[i]=NULL;             //pointer Array initialisation
     }
...
/// Date: 13/01/2026
///////////////////////////////////////////////////////////
void createDILB()//Disk Inode List Block
{
    int i=0;
    PINODE newn=NULL;
    PINODE temp =head;
...
superobj.FreeInodes--;
return i;////returning fd(file descriptor)
}
...
/// Description: It is Used to Store File details on (Actual device disk) backup file
/// Input: Nothing
/// Output:Nothing
/// Author: Atharva Vasant Kandhare
/// Date: 23/06/2026
...
void saveState()
{
    ...
    // Step 1 — Create backup file with timestamp in name(with help of time)
    sprintf(filename, "cvfs_state_backup_%ld.dat", (long)time(NULL));
    fd = open(filename, O_WRONLY | O_CREAT , 0600);//access for only owner
    if(fd == -1)
    {
    ...
    //  Save FreeInodes count
    write(fd, &superobj.FreeInodes, sizeof(int));
    ...
    {
        // Save inode number
        write(fd, &temp->InodeNumber,sizeof(int));

        // Save file name
        write(fd, temp->FileName,20);

        // Save file type
        write(fd, &temp->FileType,sizeof(int));

        // Save actual file size
        write(fd, &temp->ActualFileSize,sizeof(int));

        // Save permission
        write(fd, &temp->Permission,sizeof(int));

        // Save reference count
        write(fd, &temp->ReferanceCount,sizeof(int));

        // Save buffer content
        // If file exists — save actual content
        // If file does not exist — save empty block
        ...
        {
            char emptyBlock[MAXFILESIZE] = {'\0'};
            write(fd, emptyBlock, MAXFILESIZE);
        }
    ...
    while(1)
    {
        ...
        if(iCount==1)
        {
            ...
            else if(strcmp("help",command[0])==0)
            {
             ...
             //Project CVFS :> echo (after that enter msg)
